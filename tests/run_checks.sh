...
# (including the correction of the n = 6 claim of Bae 2002 / Bae-Choi 2003), the k = 4, 5 programs, the
# hole-chain construction, and one n = 7 value against the published count table. The decisive n = 7 values are
# checked by `make verify`.
...
check "Bae (2002) / Bae-Choi (2003) correction: {109,147,161,166,168,169} is the only admissible 6-set with max 169" \
...
if python3 -c 'import numpy' 2> /dev/null; then
    ...
    echo "FAIL  hole-chain beam search: NumPy is missing (python3 -m pip install -r requirements.txt)"
    fail=$((fail + 1))
fi
