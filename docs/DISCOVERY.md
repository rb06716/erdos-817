...
| Run by the repository owner (a human) on different hardware, Google Colab (Intel, 8 vCPUs), 2026-09-23: `verify_result.py quick` 7/7 PASS and **`critical` 7/7 PASS: all 60 count vectors N = 419…478 identical to the published table, same solution lists** (`results/external_verification/`) | the lower-bound computation reproduced on another machine by a human (same code, so this guards against environment and hardware errors, not against a shared logic error) |
