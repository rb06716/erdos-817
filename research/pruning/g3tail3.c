/*
 * g3tail3.c -- research prototype: g3fast2.c with a THREE-level tail (the last three elements are chosen from
 * D_{n-3} with shifted windows, so D_{n-2} is never materialised).  NOROOM semantics only (canonical counts).
 *   level n-2 element w: candidates of D_{n-3};
 *   level n-1 element y: y - j*w and 2y - j*w notin D_{n-3} (j = -2..2)            [10 window reads]
 *   level n   element x: x must pass the same test as y (k = 0 terms) and
 *                        x - j*w - k*y, 2x - j*w - k*y notin D_{n-3} for k != 0     [40 bit tests]
 * D_{n-3} must be exact on [-4N, 6N].
 *
 * (original header of g3fast2.c follows)
...
static int dfs(int L, int last, long long sum) {
    const u64 *D = Dbuf[L];
    if (L == n - 3) {
        int wlen = N - 1 - last;                     /* w in [last+1, N-1] */
        if (wlen < 1) return 0;
        u64 wc[24];
        candidates(D, last + 1, wlen, wc);
        int wnw = (wlen + 63) >> 6;
        for (int wi = 0; wi < wnw; wi++) while (wc[wi]) {
            int wt = __builtin_ctzll(wc[wi]);
            wc[wi] &= wc[wi] - 1;
            int w = last + 1 + 64 * wi + wt;
            nodes[L + 1]++;
            int ylen = N - 1 - w;                    /* y in [w+1, N-1] */
            if (ylen < 1) continue;
            int ynw = (ylen + 63) >> 6;
            u64 acc[24], tmp[24];
            window(D, w + 1, ylen, acc);
            for (int j = -2; j <= 2; j++) {
                if (j) { window(D, w + 1 - j * w, ylen, tmp); for (int i = 0; i < ynw; i++) acc[i] |= tmp[i]; }
                window_even(D, 2 * (w + 1) - j * w, ylen, tmp);
                for (int i = 0; i < ynw; i++) acc[i] |= tmp[i];
            }
            for (int i = 0; i < ynw; i++) acc[i] = ~acc[i];
            if (ylen & 63) acc[ynw - 1] &= (((u64)1) << (ylen & 63)) - 1;
            /* acc = legal (n-1)-th elements y; the n-th element x must also lie in acc (k = 0 terms) */
            for (int yi = 0; yi < ynw; yi++) {
                u64 yb = acc[yi];
                while (yb) {
                    int yt = __builtin_ctzll(yb);
                    yb &= yb - 1;
                    int y = w + 1 + 64 * yi + yt;
                    nodes[L + 2]++;
                    /* x candidates: bits of acc above y */
                    for (int xi = yi; xi < ynw; xi++) {
                        u64 xb = (xi == yi) ? yb : acc[xi];
                        while (xb) {
                            int xt = __builtin_ctzll(xb);
                            xb &= xb - 1;
                            int x = w + 1 + 64 * xi + xt;
                            int ok = 1;
                            for (int k = -2; k <= 2 && ok; k++) {
                                if (!k) continue;
                                for (int j = -2; j <= 2; j++) {
                                    int p = x - j * w - k * y, q = 2 * x - j * w - k * y;
                                    if (getbit(D, p < 0 ? -p : p) || getbit(D, q < 0 ? -q : q)) { ok = 0; break; }
                                }
                            }
                            if (!ok) continue;
                            nodes[L + 3]++;
...
int main(int argc, char **argv) {
    ...
    for (N = Nlo + offset; N <= Nhi; N += step) {
        ...
        needlo[n - 3] = -4 * N; needhi[n - 3] = 6 * N;
        for (int L = n - 4; L >= 0; L--) { needlo[L] = needlo[L + 1] - 2 * N; needhi[L] = needhi[L + 1] + 2 * N; }
